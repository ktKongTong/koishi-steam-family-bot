...
export const useComputedData = (
  ...
  const appsForUse=libs
      .filter(item=>
        {
          return filteredPlayer.filter(player=>item.ownerSteamids.includes(player.steamid.toString())).length > 0
        }
  ...
  const tags = appsForUse
      .flatMap(app=> app.mappedTags.map(it=>parseInt(it)).slice(0,10))
  ...
  const mappedRecentLibs = recentLibs.map(it=> {
    ...
    return {
      ...it,
      owners: it.ownerSteamids.map(it=> players.find(player => player.steamid.toString( ) == it)),
      detail
    }
  })
