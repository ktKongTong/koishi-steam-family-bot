...
export const libApi = (ctx: Context, config: Config, token: string) => {
  ...
  const host = `${config.SteamHelperAPIHost}/api/steam`
  let steamid = ''
  try {
  ...
  const getSteamFamilyGroup = () =>
    steamAPI.familyGroup.getFamilyGroupForUser(
      { steamid: BigInt(steamid), includeFamilyGroupResponse: true },
      token
    )

  const getPlaytimeSummary = (familyGroupid: bigint) =>
    steamAPI.familyGroup.getFamilyGroupPlaytimeSummary(
      { familyGroupid: familyGroupid },
      token
    )

  const getSteamFamilyGroupLibs = (familyId: bigint) =>
    steamAPI.familyGroup.getFamilyGroupShardLibrary(
      {
  ...
  const getFamilyMembers = (memberIds: string[]) =>
    steamAPI.common.getSteamPlayerLinkDetails(
      { steamids: memberIds.map((it) => BigInt(it)) },
      token
    )

  const getSteamItems = async (
    appIds: string[],
    params?: PartialMessage<CStoreBrowse_GetItems_Request>
  ) =>
    steamAPI.common.getSteamItemsById(
      params ?? {
        ids: appIds
          .map((it) => parseInt(it))
  ...
  const getSteamFamily = async () =>
    wrapperErr(() =>
      http.get<APIResp<SteamFamily>>(
        `${host}/family?access_token=${access_token}`
      )
    )
  const getFamilyLibs = async (familyId: string) =>
    wrapperErr(() =>
      http.get<APIResp<SharedLibResp>>(
        `${host}/family/shared/${familyId}?access_token=${access_token}`
      )
    )
  const getFamilyMember = async (memberIds: string[]) =>
    wrapperErr(() =>
      http.get<APIResp<SteamPlayers>>(
        `${host}/player/${memberIds.join(',')}?access_token=${access_token}`
      )
    )
  ...
  const getOnePlayersSteamWishes = async (id: string) => {
    ...
    while (hasMore) {
      const res: Record<string, any> = await withRetry(() =>
        getSteamWishesByPage(id, page)
      )
      if (res) {
