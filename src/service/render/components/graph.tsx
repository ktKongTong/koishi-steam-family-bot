...
export default function Graph({
  ...
  const { cntData, dicts, playtimeData, mappedRecentLibs } = useComputedData(
    libs,
    recentLibs,
    filteredPlayer,
    players,
  ...
  return (
    ...
    >
      <div
        className={
          'bg-blend-darken dark bg-black/[.6] z-10 rounded-lg flex flex-col items-center justify-center mx-auto space-y-2 p-1 md:p-4 w-full h-full'
        }
      >
        ...
        <div
          className={
            'grid grid-cols-2 sm:grid-cols-3 justify-evenly items-center w-full gap-1 mx-auto px-4 place-items-center'
          }
        >
          {players.map((player) => (
            <PlayerItem key={player.steamid} player={player} />
          ))}
        </div>
        <Piegraph countData={cntData} style={{ height: 400, width: 1000 }} />
        <PlaytimeGraph
        ...
        <div
          className={
            'grid grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-2 md:gap-2 lg:gap-2 '
          }
        >
          {mappedRecentLibs.map((app) => (
            <Game key={app.appId} game={app} players={players} />
          ))}
        </div>
      </div>
      <img
