...
const libTagSyncer = (ctx:Context,config:Config) => async ()=> {
  ...
  const totalLibItem = (await ctx.database.get('SteamFamilyLib',
    // {}))
    (row)=> {
    return $.and(
      $.or(
  ...
  const libTags = _.chunk(libsId, 30).flatMap(async (chunk)=> {
    try {
      const steamItems = await api.Steam.getSteamItems(chunk.map(it=> it.toString()),{
        ids: chunk.filter(it => !Number.isNaN(it))
          .map(it=>({appid: it})),
        context: {
          language: 'schinese',
          countryCode: 'US',
  ...
  logger.info(`lib tags sync finished, synced ${res.filter(it=>!it.tagSynced).length} items`)
}
